...
The speed limit and average speed are both given in km/hr"""


#Answer:

def time_saved(speedLimit, AverageSpeed , Distance):
    # returns time you saved by going over speed limit on particular distance (speedLimit, AverageSpeed , Distance)
    RoadTimeInSpeedLimit=((Distance/speedLimit)*60)
    RoadTimeWhileSpeeding=((Distance/AverageSpeed)*60)
    timeSaved=round((RoadTimeWhileSpeeding-RoadTimeInSpeedLimit),1)
    return abs(timeSaved)
